...
class Agent(nn.Module):
    def __init__(self, obs_dim, act_dim):
        ...
        self.fc4_critic = nn.Linear(self.FEATURE_NUM * self.s_dim[0] + 12, self.FEATURE_NUM)
        ...
        self.fc4_actor = nn.Linear(self.FEATURE_NUM * self.s_dim[0] + 12, self.FEATURE_NUM)
        self.pi_head = nn.Linear(self.FEATURE_NUM, act_dim)

    def get_value(self, x, vae_input=None):
        if len(x.size()) == 2:
            x = x.unsqueeze(0)
        ...
        critic_merge_net = torch.cat([critic_split_0, critic_split_1, critic_split_2, critic_split_3, critic_split_4, critic_split_5, vae_input], 1)
    ...
    def get_max_action_and_value(self, x, action=None, vae_input=None):
        if len(x.size()) == 2:
            x = x.unsqueeze(0)
        # Actor Net
        ...
        actor_merge_net = torch.cat([actor_split_0, actor_split_1, actor_split_2, actor_split_3, actor_split_4, actor_split_5, vae_input], 1)
        pi_net = F.relu(self.fc4_actor(actor_merge_net))
        logits = self.pi_head(pi_net)

        # Critic Net
        value = self.get_value(x, vae_input=vae_input)
